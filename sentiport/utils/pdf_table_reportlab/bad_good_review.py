...
def get_translated_language(DATAFRAME):
  ...
  DATAFRAME = pd.concat([DATAFRAME,NEG_DATAFRAME]).drop_duplicates(['review','rating','word_count_score'],keep='last').sort_values('word_count_score', ascending=False)
  return DATAFRAME
...
def get_sentiment_score(bad_review):
  ...
  sentiment_df['score'] = sentiment_df['word_count_score']*sentiment_df['polarity']
  sentiment_df = sentiment_df.sort_values('score', ascending=True).reset_index(drop=True)
  return sentiment_df
...
def get_good_review(dataframe):
  dataframe = dataframe.reset_index()
  good_review = dataframe[dataframe['rating']==5].sort_values('word_count_score', ascending=False)#.reset_index()
